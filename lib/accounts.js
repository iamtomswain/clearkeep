...
// ── Accounts ────────────────────────────────────────────────────────────────
function publicRecord(a) { return { id: a.id, address: a.address, provider: a.provider, swatch: a.swatch, scope: a.scope || "" }; }
...
function addAccount(rec) {
  ...
  if (rec.scope) stored.scope = rec.scope; // OAuth scope granted (drives write capability)
